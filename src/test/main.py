...
def main():
    ...
    #detection_area.draw_image(data=detection_img0, location=(0, 0))
    
    #sg.Image(filename='', size=video_size, enable_events=True, key='-IMAGE_TRACKING-')
    #tracking_area = window['-TRACKING_GRAPH-']
    #resized = cv2.resize(last_frame, video_size, fx=1, fy=1, interpolation = cv2.INTER_LINEAR)
    #tracking_img0 = cv2.imencode('.png', resized)[1].tobytes()    
    #tracking_area.draw_image(data=tracking_img0, location=(0, 0))
    ...
    while True:
        ...
        if  cap.isOpened and is_playing: 
            ret, frame = cap.read()
            is_playing = ret
            is_tracking, tracking_bbox = tracker.update(frame)
        else:
            is_playing = False
        ...
        #detection_area.draw_image(data=imgbytes, location=(0, 0))
        #tracking_area.draw_image(data=imgbytes, location=(0, 0))
